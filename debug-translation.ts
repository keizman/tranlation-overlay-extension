 ...
 * 用于独立测试和分析位置计算问题
 *
 * 运行方式: npx ts-node debug-translation.ts
 */
...
function parseResponse(response: string): ReplacementPair[] {
...
function validateWordBoundary(text: string, word: string): boolean {
...
interface Replacement extends ReplacementPair {
...
function addPositions(text: string, pairs: ReplacementPair[]): Replacement[] {
...
function applyReplacements(text: string, replacements: Replacement[]): string {
  ...
  for (const rep of sorted) {
    ...
    if (middle !== rep.original) {
      console.error(
        `❌ 位置错误: 期望"${rep.original}" 实际"${middle}" @ ${start}-${end}`,
      );
    }
...
for (const rep of replacements) {
  const extractedText = originalText.substring(
    rep.position.start,
    rep.position.end,
  );
  const match = extractedText === rep.original ? '✅' : '❌';
  console.log(
    `   ${match} "${rep.original}" @ ${rep.position.start}-${rep.position.end} (提取: "${extractedText}")`,
  );
}
